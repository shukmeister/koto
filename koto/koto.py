...
Koto commands:
	...
	status    Show a history of communications with a contact (or all)
...
def main():
	...
	if arguments['add']:
		if arguments['<email>']:
			...
			db.addEmail(arguments['<email>'], arguments['<firstname>'].capitalize(),
						arguments['<lastname>'].capitalize())
		else:
			# print("Adding " + str(arguments['<firstname>']) + " " + str(arguments['<lastname>']) + " to database")
			db.insertDB(arguments['<firstname>'].capitalize(), arguments['<lastname>'].capitalize())
		...
		if arguments['all']:
			...
			if matches > 1:
				...
				print(
				"From: " + arguments['<firstname>'].capitalize() + " " + lastName.capitalize() + " (" + emailID + ")")
